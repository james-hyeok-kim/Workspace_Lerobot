"""Stage 5b — SnapFlow (NFE=1) + LLM-only W4A4.

Loads the 30K-trained student checkpoint, applies:
  - LLM R1 + shared R3 rotations (same as Stage 3/4)
  - W4A4 quantization on LLM backbone ONLY (DiT stays FP16)
  - Eval with NFE=1

Hypothesis: DiT (action generator) is more sensitive to quantization
than the LLM (vision/language processor). Keeping DiT at FP16
while quantizing only the LLM may preserve action quality.

Usage:
    python scripts/stage5b_snapflow_llm_w4a4.py
"""
...
def _load_student_weights(policy, ckpt_path: str):
    ...
    print(f"[INFO] Student loaded: {ckpt_path}")
    if missing:
...
def _apply_quarot(policy, device: str):
    """LLM R1 + shared R3 (identical to Stage 3/4)."""
    ...
    print(f"[INFO] RMSNorm fused: LLM={n_fused}, DiT=0 (AdaLN)")
    ...
    with torch.no_grad():
        ...
        for layer in expert_layers:
            _apply_r3_to_layer(layer, shared_head_signs)
    print("[INFO] LLM R1 + shared R3 applied")


def _build_llm_only_w4a4_config(ohb_manifest: dict) -> dict:
    """W4A4 config that targets ONLY LLM layers, excluding DiT/expert."""
    fp16 = {"enable": False}
    w4 = {"num_bits": 4, "block_sizes": {-1: 128}, "enable": True}
    a4 = {"num_bits": 4, "axis": None, "enable": True}

    config = {
        "quant_cfg": {
            # Quantize LLM backbone layers
            "*language_model*weight_quantizer": w4,
            "*language_model*input_quantizer": a4,
            # Quantize vision tower (SigLIP) too — large but less critical
            "*vision_tower*weight_quantizer": w4,
            "*vision_tower*input_quantizer": a4,
            # Quantize multi-modal projector
            "*multi_modal_projector*weight_quantizer": w4,
            "*multi_modal_projector*input_quantizer": a4,
            # Keep DiT/expert at FP16
            "*gemma_expert*weight_quantizer": fp16,
            "*gemma_expert*input_quantizer": fp16,
            # Keep action projections FP16
            "*action_in_proj*": fp16,
    ...
    # OHB: protect outlier layers from quantization
...
def _apply_llm_only_w4a4(policy, device: str) -> dict:
    """Apply W4A4 to LLM only; DiT stays FP16."""
    ...
    quant_config = _build_llm_only_w4a4_config(ohb_manifest)

    # Calibration via HDF5 data
            ...
            with torch.no_grad():
                for batch in loader:
                    ...
                    try:
                        ...
                        get_velocity(model.model, images, img_masks, tokens, masks, noise, t)
    ...
    print("[INFO] Applying W4A4 to LLM only (DiT stays FP16)...")
    mtq.quantize(policy, quant_config, forward_loop=forward_loop)
    print("[INFO] LLM-only W4A4 done.")
    return ohb_manifest
...
def main():
    ...
    parser.add_argument("--output_dir", default="results/stage5b")
    ...
    policy_cfg.compile_model = False   # avoid torch.compile clash with MTQ QuantizedLinear
    ...
    # Fix transformers 4.57.6 API in eager mode
    _patch_embed_image(policy)

    print(f"[INFO] Loading student weights from {args.student_ckpt}")
    _load_student_weights(policy, args.student_ckpt)

    print("\n[INFO] === Stage 5b: SnapFlow (NFE=1) + LLM R1+R3 + LLM-only W4A4 ===")
    _apply_quarot(policy, args.device)
    ohb_manifest = _apply_llm_only_w4a4(policy, args.device)
    ...
    print(f"\n[INFO] Evaluating {len(task_ids)} tasks × {args.n_episodes} ep (NFE=1, LLM W4A4)...")
    with torch.no_grad():
    ...
    print(f"\n{'='*60}")
    print(f"[STAGE 5b RESULT] pc_success={pc:.1f}%  avg_sum_reward={rw:.4f}")
    print(f"  Stage 0 baseline: 94.6% (NFE=10)")
    print(f"  Stage 1 SnapFlow: 86.0% (NFE=1, no quant)")
    print(f"  Stage 5b:         {pc:.1f}% (NFE=1, LLM W4A4, DiT FP16)")
    print(f"  Config: SnapFlow NFE=1 + LLM R1+R3 + LLM W4A4(group=128) + DiT FP16")
    ...
    result = {
        "stage": "stage5b_snapflow_llm_w4a4",
        "snapflow": {"enabled": True, "nfe": 1, "ckpt": args.student_ckpt},
        "quarot": {"r1_llm": True, "r1_dit": False, "r3_shared": True},
        "w4a4": {"scope": "llm_only", "weight": "int4", "activation": "int4",
                 "group_size": 128, "dit": "fp16", "n_ohb": len(ohb_manifest)},
        "task_ids": task_ids,
        "n_episodes": args.n_episodes,
        "overall": overall,
        "eval_info": eval_info,
