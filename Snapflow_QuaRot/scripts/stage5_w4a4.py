...
def _apply_w4a4(policy, env_cfg, device: str):
    ...
    개선:
    - per-token activation (axis=0) — 논문: "per-token symmetric"
    - GPTQ 시도, 실패 시 RTN fallback
    - clipping_ratio=0.9 (논문 기본값)
    - R4 online Hadamard (fast_hadamard_transform 있으면)
    """
    ...
    try:
        ...
        use_r4 = False
        print("[WARN] fast_hadamard_transform not installed → R4 disabled")

    # GPTQ 시도, 실패 시 RTN fallback
    # 논문: GPTQ가 RTN보다 ~2pp 더 정확
    try:
        import modelopt.torch.quantization as _check_mtq
        # GPTQ 지원 여부 확인
        from quant.w4a4_recipe import build_w4_weight_only_config
        algorithm = "gptq"
        print(f"[INFO] Algorithm: GPTQ (논문 권장)")
    except Exception:
        algorithm = "max"
        print(f"[INFO] Algorithm: max (RTN fallback)")

    from quant.w4a4_recipe import build_w4_weight_only_config
    quant_config = build_w4_weight_only_config(
        group_size=128,
        ohb_manifest=ohb_manifest if ohb_manifest else None,
        algorithm=algorithm,
    )

    # Calibration via HDF5 dataset (64 batches)
    ...
    print(f"[INFO] MTQ quantization ({algorithm.upper()}, W4 weight-only, 64 calib batches)...")
    try:
        mtq.quantize(policy, quant_config, forward_loop=forward_loop)
        print("[INFO] Quantization applied.")
    except Exception as e:
        print(f"[WARN] {algorithm.upper()} failed ({e}), falling back to RTN...")
        from quant.w4a4_recipe import build_w4_weight_only_config
        quant_config_rtn = build_w4_weight_only_config(
            group_size=128,
            ohb_manifest=ohb_manifest if ohb_manifest else None,
            algorithm="max",
        )
        mtq.quantize(policy, quant_config_rtn, forward_loop=forward_loop)
        print("[INFO] RTN quantization applied (fallback).")
        algorithm = "max"
...
def main():
    ...
    print(f"  W4 weight-only (group=128, GPTQ) + {len(ohb_manifest)} OHB layers FP16 | activations: FP16")
