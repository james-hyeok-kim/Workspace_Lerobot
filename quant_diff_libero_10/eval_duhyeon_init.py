...
def main():
    parser = argparse.ArgumentParser(
        description="Duhyeon nvfp4_bmm eval — WITH init state controller (0417 style)"
    )
    ...
    # Install init state controller (0417 style)
    suite_name_tmp = next(iter(envs_dict))
    for tid in task_ids:
        _install_init_state_controller(envs_dict[suite_name_tmp][tid])
    print("[INFO] _install_init_state_controller applied to all task envs")
