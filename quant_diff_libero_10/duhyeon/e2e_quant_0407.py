...
def _make_hook_int(w_bits, a_bits, quantize_output):
    def hook(module, inp, output):
        ...
        result = y.to(x.dtype).reshape(orig[:-1] + (w.shape[0],))
        if quantize_output:
            r2d    = result.float().reshape(-1, result.shape[-1])
            result = fq_act_per_token(r2d, a_bits).to(result.dtype).reshape(result.shape)
        return result
    return hook
...
def _make_hook_mx(w_fmt, a_fmt, block_size, scale_type, quantize_output):
    def hook(module, inp, output):
        ...
        x2d  = x.float().reshape(-1, x.shape[-1])
        w    = module.weight.float()
        w_q  = mx_block_quant(w,   w_fmt, block_size, scale_type)
        x_q  = mx_block_quant(x2d, a_fmt, block_size, scale_type)
        y    = x_q @ w_q.T
        if module.bias is not None:
            y = y + module.bias.float()
        result = y.to(x.dtype).reshape(orig[:-1] + (w.shape[0],))
        if quantize_output:
            r2d    = result.float().reshape(-1, result.shape[-1])
            r_q    = mx_block_quant(r2d, a_fmt, block_size, scale_type)
            result = r_q.to(result.dtype).reshape(result.shape)
        return result
    return hook
